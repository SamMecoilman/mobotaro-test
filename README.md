...
- SPTは以下のいずれかに自由に割り振れる：
  ...
  - 移動速度
  - 攻撃速度
- XP / レベル / SPT / 最終ステータスは **サーバ権威（authoritative）**で確定する
- 運（Luck）は主にクリティカル発生率など、確率系要素に影響する
...
### ステータス表示（HP / SP）
- HP / SP はゲージ表示を基本とする
- ゲージは画面端に固定配置し、常に視認できるようにする
- 数値（現在値 / 最大値）は補助情報として小さく表示する
  - ゲージの上部または下部に配置
  - 主張しすぎず、確認用途に留める
...
※ 攻撃方向の最終判定・正規化・連射間隔（攻撃速度含む）はサーバ権威で行う。
...
- 到達点：
  ...
  - 背景画像を map/school2.png に差し替え、4倍表示とカメラ境界を画像サイズに合わせた
...
- 次の目標（直近）：
  ...
  - 操作ガイドを含むUIの確定（モバイルは最小HUD＋必要機能はパネル/折り畳みで呼び出し）
- 次の目標（中期）：
