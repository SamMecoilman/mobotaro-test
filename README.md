...
- 敵：
  - HPを持ち、ダメージで減少する
  - HPが0になると消滅（撃破）
- 敵撃破時、**ランダムでコインをドロップ**する
- コインドロップ時に `client/public/audio/se/drop_coin.mp3` を再生する
- プレイヤー：
  - HPを持ち、ダメージで減少する
...
- UIは **カメラ非追従（画面固定）**
- 背景：
  - `map/schoolMap/ComfyUI_00019_.png` を使用
  - 表示スケール：**1倍**
  - マップ端まで移動可能（サーバ側クランプ対応）
- プレイヤーのスポーン／リスポーン初期位置は **画面中心ではなく、マップ中心付近をランダムに設定**
...
### UIスケール（端末別）
- **UIのみ** 1920x1080 を基準にスケールする（マップ表示は対象外）
- 画面サイズに応じて UI を縮小し、比率は維持する
...
- BGM/SEアセットは `client/public/audio/` 配下に配置する
...
- 到達点：
  ...
  - 背景
    ...
    - 表示スケール：1倍
    - マップ端まで移動可能（サーバ側クランプ）
  - プレイヤーのスポーン／リスポーン初期位置は画面中心ではなく、マップ中心付近をランダムに設定
  ...
  - UI（PC）
    - DOM HUD は削除/非表示のまま維持、Phaser内UI中心へ移行中
    - 右側オーバーレイUI（inventory/skill）はPCのみ表示、I/Kで開閉
    - 右パネルは Container 化済み（右端固定・open/closed座標計算）
  - 左メインパネル（Phaser）
    - main_panel_base + XP/HP/SP マスク画像で常設UIを実装
    - スケールは MAIN_PANEL_SCALE=0.45
    - HPマスクは実HPに同期、SPマスクも実SPに同期
    - XPマスクは実XP比率に同期
  - デバッグ
    - HPソース不一致検出用に DEBUG_HP_DIVERGENCE を追加（デフォルト false）
  ...
  - キャラクター画面（UI）
    ...
    - ステータス +1/+5 の当たり判定デバッグ用オーバーレイ（緑/赤）を表示中
  ...
  - BGM
    - schoolエリアBGMをループ再生
      - client/public/audio/bgm/Pixel Study Hall.wav
    - 初期音量：50%
    - オプション画面から音量調整可能
...
- 次の目標（直近）：
  - ステータス反映のバランス調整（TUNING値の微調整のみで回す）
    - 初期攻撃速度 / 伸び幅 / 下限値の体感確認
    - MOVE SPD/ATK/DEF/LUCK/RANGE も同様に「体感が出るが壊れない」範囲へ
